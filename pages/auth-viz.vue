...
const scenarios = {
  ...
  request: {
    ...
    desc: 'Before each request, check not only token presence but also its TTL. If less than 1/3 of lifetime remains (< 3 min of 10) - queue the request, do preventive refresh, then execute.',
    data: [
      // Top row - flow start
      ...
      // Central flow
      { id: 'check_token', type: 'logic', position: { x: 350, y: 150 }, label: 'JS: Has Access Token?' },
      { id: 'check_ttl', type: 'logic', position: { x: 350, y: 280 }, label: 'TTL > 1/3 Life?' },
      ...
      // Middle path - TTL Queue
      { id: 'queue_ttl', type: 'queue', position: { x: 100, y: 430 }, label: 'TTL Queue' },
      { id: 'pre_refresh', type: 'api', position: { x: 350, y: 430 }, label: '🔄 Pre-flight Refresh' },
      ...
      // Bottom - Logout
      { id: 'login', type: 'default', position: { x: 350, y: 750 }, label: '🚪 Logout', class: 'tw-bg-red-600 tw-text-white tw-border-none' },
      ...
      // Token checks
      { id: 'e4', source: 'check_token', target: 'check_ttl', label: 'Yes (Found)', style: { stroke: '#10b981' } },
      { id: 'e5', source: 'check_ttl', target: 'do_req', label: 'Fresh (> 3m)', style: { stroke: '#10b981' } },
      { id: 'e6', source: 'check_token', target: 'pre_refresh', label: 'Missing', style: { stroke: '#f87171' }, type: 'step' },
      
      // TTL Queue path
      { id: 'e7', source: 'check_ttl', target: 'queue_ttl', label: 'Low TTL', style: { stroke: '#fbbf24', strokeWidth: 2 } },
      { id: 'e8', source: 'queue_ttl', target: 'pre_refresh', label: 'Wait', animated: true, style: { stroke: '#fbbf24', strokeWidth: 2 } },
      { id: 'e9', source: 'pre_refresh', target: 'do_req', label: 'Got Token', style: { stroke: '#10b981', strokeWidth: 2 }, type: 'step' },
      { id: 'e10', source: 'pre_refresh', target: 'login', label: 'Fail', style: { stroke: '#f87171' } },
      
      // Request execution
      { id: 'e11', source: 'do_req', target: 'success', label: '200 OK', style: { stroke: '#10b981' } },
      { id: 'e12', source: 'do_req', target: 'check_401', animated: true, style: { stroke: '#94a3b8' }, type: 'step' },
      
      // 401 Queue path (linear horizontal)
      { id: 'e13', source: 'check_401', target: 'success', label: '200 OK', style: { stroke: '#10b981' }, type: 'step' },
      { id: 'e14', source: 'check_401', target: 'queue_401', label: 'Yes (401)', style: { stroke: '#f87171', strokeWidth: 2 } },
      { id: 'e15', source: 'queue_401', target: 'retry_refresh', label: 'ONE Refresh', animated: true, style: { stroke: '#f87171', strokeWidth: 3 } },
      { id: 'e16', source: 'retry_refresh', target: 'do_req', label: 'Retry', style: { stroke: '#60a5fa', strokeWidth: 2, strokeDasharray: '5,5' }, type: 'smoothstep' },
      { id: 'e17', source: 'retry_refresh', target: 'login', label: 'Fail', style: { stroke: '#f87171' }, type: 'smoothstep' }
    ]
  },
